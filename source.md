...
模型已经能主动完成某项分析后，Skill中原有的逐步指导就可能不再必要，但流程要求的报告、文件读取和工具调用仍会消耗Token和时间。Skill需要持续回答：在当前模型、任务和环境下，多加这一步，究竟改善了什么结果？

判断某个步骤是否还有必要，要先看它在任务中起什么作用。模型已经能自行完成的分析，可以减少对顺序和轮次的规定，并检查相关报告和读写是否仍有必要。做法固定的操作，可以整合为脚本复用；项目的最新信息和具体交付要求仍需提供，编译和测试结果仍需实际运行后确认。这样逐项检查，才能在减少重复工作的同时，保留完成任务所需的支持。调整后再比较质量与开销，确认精简后的流程仍能满足交付要求。
...
这次单元测试实验具体检验了这样的调整：减少重复说明和中间报告，将固定操作整合为脚本，并补充交付前的检查要求。调整后，记录中的累计Token下降67.9%，平均任务耗时下降40.2%，整体质量评分从93.68变为94.03。新版以更低的执行开销取得了接近的整体评分，说明旧流程中的一部分工作可以精简。

## 实验背景与研究问题

本次实验使用三个Skill，分别指导模型生成单元测试、分析测试的有效性，以及根据反馈修改和完善测试。旧版的重点是让模型按完整流程完成一次单测任务：生成测试后，检查测试是否有效，根据编译和运行结果修复问题，再重新验证。实际使用中，又暴露出重复读取源码、中间报告反复维护、部分验证轮次没有带来有效变化等过程负担。
...
Apollo组中，无Skill模型已经取得较高评分，完整流程把Token提高约5.1倍，没有观察到评分增加。MyBatis-3组中，旧版Skill模型则高5.56分。同一套流程在不同任务组里带来了不同的收益，Skill可以据此调整支持的范围与深度，把额外投入用在更需要帮助的任务上。
...
新版平均每任务约124.5万Token，仍为无Skill组的2倍，但已明显低于旧版的387.6万Token。这轮优化保留了有用的指导与执行能力，同时减少了重复说明、报告和工具调用带来的开销。
...
精简流程时，可以减少固定的分析轮次和报告要求，让模型按任务需要选择做法。交付前仍需检查产物是否符合要求：编译和运行用于确认代码与测试能否正常执行，测试内容评估则关注覆盖是否充分、断言是否有效。模型可以自行安排完成任务的过程，但是否完成任务，仍要由这些检查结果来确认。
...
这次实践中，运行记录和评测结果被用于修改Skill，经过验证的改动再写回流程，供后续任务使用。随着模型进步，过去需要逐步指导的工作可以更多地交给模型，工程师则把精力放在明确要求、完善工具和验证结果上。Skill自进化最终要沉淀的，是持续检验和更新工程经验的能力，让积累下来的方法跟得上模型的进步。
