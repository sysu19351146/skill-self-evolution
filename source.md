...
![模型升级后重新评测当前Skill，根据运行记录修改，通过新旧版本比较保留有效改动。](assets/04-information-lifecycle.png)

*图4：模型升级后，重新评测当前Skill，结合运行反馈修改规则，再将通过评测的改动写入新版本。整个过程沿用原有的交付要求。*
...
工程师的经验，也包括判断什么时候可以少指导一些。模型尚不熟悉的工作，可以用规则和工具帮助它完成；已经能够独立完成的工作，就通过评测确认，再删去重复指导。**Skill的长期价值，在于随着模型进步，用更少的额外步骤帮助它稳定完成任务。**工程经验在这个过程中持续更新，既保留解决问题的方法，也逐步放下已经不再需要的做法。
