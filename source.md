...
这次单元测试实验给出了一个具体切口：调整流程后，记录中的累计Token下降67.9%，平均任务耗时下降40.2%，整体质量评分从93.68变为94.03。它支持重新审视旧流程的必要性。由于三组使用同一模型标识，这些结果无法证明哪些能力由后训练内化，也不能把全部节省归因于模型进步。

![按信息与操作的性质分配职责：通用分析可复评，固定操作可脚本化，当前事实与交付证据需要获取。](assets/01-model-skill-boundary.png)

*图1：Skill职责调整的判断框架；图中分类用于分析规则，不表示本次实验测出了模型的训练变化。*
...
三组运行日期不同，现有记录不足以确认所有环境条件、随机性与重试成本完全受控。质量报告也没有提供可用于本次复核的逐任务评分，因此以下分差只作描述性比较。Token包含各轮输入与输出的累计量，其中存在缓存命中；它衡量会话处理规模，不能直接换算成等比例的账单变化。
...
Apollo组中，无Skill已经取得较高评分，完整流程把Token提高到约5.1倍，没有观察到评分增加。MyBatis-3组中，旧版则高5.56分。0.24分的差异不足以判断Apollo组谁更好；两组的收益差异也可能涉及项目结构、任务类型和评分空间。现有材料没有统一的难度量表，不宜直接推出简单任务不需要Skill、复杂任务需要Skill。
...
> **RA2：**新版相对旧版的累计会话Token下降67.9%，平均耗时下降40.2%，整体报告评分相差+0.35分。开销改善覆盖了多数任务；整体分数接近，仍不能证明逐任务质量没有退化。
...
新版平均每任务约124.5万Token，仍为无Skill组的2倍。它降低了为额外支持付出的代价，并没有消除这笔代价。就本次汇总结果看，新版更值得进入后续验证；是否采用，还需要检查具体失败与质量退化。

![三组报告评分、会话Token与平均耗时对比，质量分差仅作描述性比较。](assets/02-quality-cost-comparison.png)

*图2：三组各140个对应任务。评分为既有报告值；Token为记录中的会话累计输入与输出；耗时为单任务执行时长的平均值。缺少逐任务质量分与重复运行，不能据此判断显著性或质量等价。*

逐任务配对后，新版在135个任务上Token更少，在126个任务上耗时更短，说明开销改善覆盖了多数任务。但也有5个任务Token增加、14个任务耗时增加。例如，一个MyBatis-3任务的Token接近旧版的两倍。缺少对应质量分和轨迹分析时，既不能说这是必要的质量投入，也不能断言新版发生了流程退化。

平均值适合判断整体投入，负例适合寻找下一轮修改方向。版本采用还要关心尾部风险：少数关键任务的退化，可能比许多普通任务省下的Token更重要。仅以平均分和总Token筛选版本，会遗漏这一层判断。
