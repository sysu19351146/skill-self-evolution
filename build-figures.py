...
def information_lifecycle():
    fig, ax = canvas(4, "执行产生反馈，修改接受独立验收", "可迁移的方法设计建议：人工与模型协同更新；不表示已实现全自动进化。")
    for x, title, body in [(.8, "当前方法", "规则、脚本与适用条件\n执行时读取当前版本"),
                           (6.05, "任务执行", "按需获取当前事实\n生成产物并实际验证"),
                           (11.3, "结果与运行记录", "当前产物、验证范围与条件\n保留失败及过程开销")]:
        card(ax, x, 4.7, 3.9, 1.87, title, body, BLUE, "#F0F5FA", 19)
    for x, title, body in [(.8, "版本决策", "达标后采用，退化则回退\n保存旧版与修改依据"),
                           (6.05, "候选比较", "配对任务与未参与修改的任务\n检查质量退化和执行成本"),
                           (11.3, "提出修改", "人工与模型复盘运行问题\n记录预期、依据与适用条件")]:
        card(ax, x, 1.57, 3.9, 1.87, title, body, PURPLE, "#F5F2FA", 19)
    arrow(ax, (4.75, 5.65), (5.99, 5.65), "按需使用", 5.36, 5.93, BLUE)
    arrow(ax, (10, 5.65), (11.24, 5.65), "留下证据", 10.63, 5.93, BLUE)
    arrow(ax, (13.25, 4.65), (13.25, 3.5), "反馈用于复盘", 14.15, 4.05, PURPLE)
    arrow(ax, (11.24, 2.5), (10, 2.5), "提交候选", 10.63, 2.8, PURPLE)
    arrow(ax, (5.99, 2.5), (4.75, 2.5), "按结果决定", 5.36, 2.8, PURPLE)
    arrow(ax, (2.75, 3.5), (2.75, 4.65), "更新当前版本", 1.78, 4.05, GREEN)
    text(ax, 8.0, 4.32, "验收标准单独维护", 17, ORANGE, "bold", ha="center")
    arrow(ax, (8, 4.04), (8, 3.5), "约束候选比较", 9.25, 3.83, ORANGE)
    text(ax, .8, 1.18, "验证证据有适用条件：输入、产物、环境或检查范围变化后，重新确认相关结果。", 12, MUTED)
    footer(ax, "允许方法变化，保留统一的交付要求与可回退的版本。")
